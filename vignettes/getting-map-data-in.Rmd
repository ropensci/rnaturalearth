...
countries50 <- readOGR(tempdir(), 'ne_50m_admin_0_countries', encoding='UTF-8')

download.file(file.path('http://www.naturalearthdata.com/http/',
                        'www.naturalearthdata.com/download/110m/cultural',
                        'ne_110m_admin_0_sovereignty.zip'), 
                        f <- tempfile())

unzip(f, exdir=tempdir())

sovereignty110 <- readOGR(tempdir(), 'ne_110m_admin_0_sovereignty', encoding='UTF-8')
...
file_tiny_countries110 <- paste0(dataFolder,'ne_110m_admin_0_tiny_countries.shp')
...
#advantage of using rgdal is that it keeps proj4string(countries50)
require(maptools)
require(rgdal)

#countries50 read in using rgdal above
...
tiny_countries110 <- maptools::readShapePoints(file_tiny_countries110)
